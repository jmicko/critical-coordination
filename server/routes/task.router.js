...
router.get('/project/:id', rejectUnauthenticated, (req, res) => {
  ...
  const sqlText = `SELECT task.id AS id, nlt_date, poc_fk, project_fk, scheduled_date, status_type, task_name, task_name_fk, task_status_fk, technician_info, token, tracking_id, updated_by   
                  FROM task 
