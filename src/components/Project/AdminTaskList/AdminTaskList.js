  ...
  render() {
    return (
      <div >
        ...
        {this.state.showEditTask
          ? <p><label>NLT Date:<input type='date' onChange={(event => this.handleChange(event, 'nlt_date'))} placeholder={this.props.task.nlt_date}></input></label></p>
          : <p> NLT Date: {this.dateConversion(this.props.task.nlt_date)} </p>}
        {this.state.showEditTask
          ? <label>Status:<select onChange={(event) => this.handleChange(event, 'task_status')}>
            <option value=''></option>
        ...
        <center>
          {this.state.showEditTask ?
            <>  <button className="btn" onClick={this.fieldValidation}>Save</button> <button className="btn" onClick={this.showEditTask}>Cancel</button><button className="btn" onClick={this.delete}>Delete</button> </> :
            <p> <button className="btn" onClick={this.showEditTask}>Edit</button>  </p>
          }
          <p><button className="btn" onClick={(event) => this.confirmSend(this.props.task.id)}>Notify Contractor via Email</button></p>
